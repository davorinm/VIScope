...
public func allClose(_ x:[Double], y:[Double], tol:Double = 3e-7) -> Bool {
    ...
    if x.count == y.count {
        ...
        // Take abs value
        var xMinusYout = [Double](repeating: 0.0, count: N)
        vDSP_vabsD(&xMinusY, 1, &xMinusYout, 1, vDSP_Length(N))

        var maximum:Double = 0
        vDSP_maxvD(&xMinusYout, 1, &maximum, vDSP_Length(N))
...
public func allClose(_ x:[Float], y:[Float], tol:Float = 3e-7) -> Bool {
    ...
    if x.count == y.count {
        ...
        // Take abs value
        var xMinusYout = [Float](repeating: 0.0, count: N)
        vDSP_vabs(&xMinusY, 1, &xMinusYout, 1, vDSP_Length(N))

        var maximum:Float = 0
        vDSP_maxv(&xMinusYout, 1, &maximum, vDSP_Length(N))
